...
> = {
  mint: {
    ...
    dot: "bg-[#5EE287]",
    progress: "bg-[linear-gradient(90deg,#5EE287,#8AF5AC)]",
  },
  sky: {
...
function MetricCardVisualBlock({ visual }: { visual: MetricCardVisual }) {
  ...
  if (visual.kind === "ring") {
    return (
      <div className="mt-4 rounded-[20px] border border-[#243242] bg-[#101822] p-3">
        <div className="flex items-center gap-3">
          ...
          >
            <div className="absolute inset-[6px] rounded-full bg-[#101822]" />
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#8CA1B8]">{visual.label}</p>
            <p className="mt-1 text-xs text-[#AFC1D3]">Macht auf einen Blick sichtbar, wie viel davon schon in deinem Profil steckt.</p>
          </div>
        </div>
      </div>
    );
...
export function MetricCard({
  ...
  return (
    <div className="group flex h-full flex-col rounded-[26px] border border-[#293749] bg-[linear-gradient(135deg,rgba(18,26,38,0.98),rgba(15,22,33,0.94))] p-4 transition-all duration-300 hover:-translate-y-1 hover:border-[#5EE287]/35 hover:shadow-[0_18px_40px_rgba(0,0,0,0.26)]">
...
export function SnapshotFeatureCard({
  ...
  return (
    <div className="group flex h-full min-h-[240px] flex-col rounded-[26px] border border-[#293749] bg-[linear-gradient(135deg,rgba(18,26,38,0.98),rgba(15,22,33,0.94))] p-5 transition-all duration-300 hover:-translate-y-1 hover:border-[#5EE287]/35 hover:shadow-[0_18px_40px_rgba(0,0,0,0.26)]">
      <div className="flex items-start justify-between gap-4">
        <p className="text-xs uppercase tracking-[0.18em] text-[#8CA1B8]">{label}</p>
        <span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-[#2D3A4B] bg-[#151F2B] text-[#D9E7F3] transition-transform duration-300 group-hover:scale-105 group-hover:border-[#5EE287]/35">
          <Icon size={18} />
        </span>
      </div>
      ...
      <div className="mt-5 flex flex-1 flex-col">
        <div className="flex-1">
          ...
          <div className="mt-2 text-sm leading-relaxed text-[#AFC1D3]">{description}</div>
        </div>
        {support && <div className="mt-5">{support}</div>}
      </div>

      {footer && <div className="mt-5 border-t border-[#243242] pt-4 text-sm text-[#D6E2EF]">{footer}</div>}
    </div>
  );
}
    ...
    >
      ...
      <div className="mt-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#D6E2EF]">{badge.progressPercent}%</span>
        </div>
